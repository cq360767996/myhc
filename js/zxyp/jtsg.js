requirejs(["common", "ec", "ecPlugin"], function (sugon, ec) {
    ...
    var searchRuler = {}, mid1_3Data = [], mid2_3Data = [];
    ...
    var initLeft2 = function () {
        sugon.requestJson({
            ...
            var xData = [], yData = [], min = Number(result.data[0].value), max = Number(result.data[0].value);
            for (var i = 0; i < result.data.length; i++) {
                xData.push(result.data[i].name);
                yData.push(result.data[i].value);
                Number(result.data[i].value) < min ? min = Number(result.data[i].value) : min;
                Number(result.data[i].value) > max ? max = Number(result.data[i].value) : max;
            }
            var diff = max - min;
            max = max + diff;
            min = min - diff;
    ...
    var renderMid1_3 = function () {
        ...
        var xData = [], yData = [], startValue = 0, endValue = 100, isShow = false, markData = [], max = data[0].value;
        if (data.length > 4) {
            isShow = true;
        ...
        for (var i = 0; i < data.length; i++) {
            ...
            var resultName = '', name = data[i].name;
    ...
    var initMid2_1 = function () {
        sugon.requestJson({
            ...
            var data = result.data,chart1_data = [], chart2_data = [];
            data.map(function (val, index) {
                if (index > 0) {
                    val.map(function (v, i) {
                        $('.color' + (index + 1)).eq(i).html(Math.abs(v) + '%');
                    });
                } else {
                    $('.color1').html(val[0] + '%');
    ...
    var initRight1 = function () {
        sugon.requestJson({
            ...
            var data = result.data;
            data.map(function (val, index) {
                $('.right1 > div').eq(index).find('span').html((index + 1) + '.' + val[0]);
                var color, imgDiv;
                if (val[1] > 0) {
                    color = '#dd3232';
                ...
                $('.right1 > div').css('line-height', $('.right1 > div').height() + 'px');
                $('.right1 > div').eq(index).find('div').css('color', color).find('span')
                    .html(imgDiv);
                $('.right1 > div').eq(index).find('div').find('strong').html(Math.abs(val[1]) + '%');
            });
        });
    };
    ...
    var initRight2 = function () {
        sugon.requestJson({
            ...
            data.map(function (val, index) {
                ...
                var $div = $('<div></div>');
                $div.append($('<span></span>').attr('title', val[0]).html((index + 1) + '.' + val[0]))
                    .append($('<strong></strong>').css('color', color).html(val[1] + '%'));
