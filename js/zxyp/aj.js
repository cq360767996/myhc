...
requirejs(["common", "ec", "ecPlugin", 'jqcloud'], function (sugon, ec) {
    ...
    var drawZhpjzsfx = function (data, id) {
        ...
        for (var i = 0; i < data.length; i++) {
            if (zhpjzsfxId == data[i].id && id) {
                xData.push({text: data[i].name + '\n' + data[i].value + '%', id: data[i].id, max: 100, color: 'red'});
            } else {
                xData.push({text: data[i].name + '\n' + data[i].value + '%', id: data[i].id, max: 100});
            }
    ...
    var getHjwtfx = function (id, hjwtId) {
        ...
        sugon.requestJson({
            ...
            if (!sugon.isPublished || (result.timestamp == searchRuler.timestamp && result.data.length > 0)) {
    ...
    var drawZhpjzsfxTrend = function (result) {
        ...
        var xData = [], yData1 = [], yData2 = [], data2Max = 100, data2Min = data2[0].value;
    ...
    var createAjbljtwtfxDom = function (data, $body, chartId) {
        $body.empty();
        $('<div class="tab-row"/>').append($('<div class="tab-td" style="width: 60px;">排行</div>'))
            .append($('<div class="tab-td" style="width: 140px;">问题标签</div>'))
            .append($('<div class="tab-td" style="width: 60px;">数量</div>'))
            .append($('<div class="tab-td" style="width: calc(50% - 130px);">问题走势</div>'))
            .append($('<div class="tab-td" style="width: calc(50% - 130px);">执行规范</div>')).appendTo($body);
        for (var i = 0; i < data.length; i++) {
            $('<div class="tab-row"/>').append($('<div class="tab-td" style="width: 60px;">' + data[i].ph + '</div>'))
                .append($('<div class="tab-td" style="width: 140px;">' + data[i].wtbq + '</div>'))
                .append($('<div class="tab-td" style="width: 60px;">' + data[i].sl + '</div>'))
                .append($('<div class="tab-td" id="' + chartId + i + '" style="height:30px; margin:5px 0;width: calc(50% - 130px);"></div>'))
                .append($('<div class="tab-td tab-td-click" top-id="' + data[i].id +
                    '" style="text-align: left; width: calc(50% - 130px);"><img src="../../img/zxyp/aj/file.png"><span>' +
                    data[i].zxgf + '</span></div>')).appendTo($body);
        }
    };
    ...
    var renderZsmybDom = function (index, data) {
        ...
        switch (level) {
            case '2' :
                $this.append($('<div class="view-right-bottom-row"/>').append($img3).append($('<span/>').html(data.value1)))
                    .append($('<div class="view-right-bottom-row"/>').append($('<img src="../../img/zxyp/aj/top_transparent.png">')).append($('<span/>').html(data.value2)))
                    .append($('<div class="view-right-bottom-row"/>').append($('<img src="../../img/zxyp/aj/top_transparent.png">')).append($('<span/>').html(data.value3)));
                break;
            case '1' :
                $this.append($('<div class="view-right-bottom-row"/>').append($img2).append($('<span/>').html(data.value1)))
                    .append($('<div class="view-right-bottom-row"/>').append($img3).append($('<span/>').html(data.value2)));
                break;
            default :
                $this.append($('<div class="view-right-bottom-row"/>').append($img1).append($('<span/>').html(data.value1)))
                    .append($('<div class="view-right-bottom-row"/>').append($img2).append($('<span/>').html(data.value2)));
                break;
        }
    };
            ...
            series: [
                {
                    ...
                    axisLine: { // 坐标轴线
                        ...
                        lineStyle: { // 属性lineStyle控制线条样式
                            color: [[0.98, new ec.graphic.LinearGradient(
                                0, 0, 1, 0,
                                [
    ...
    var drawPopShlAjzhpjzsfx = function (result, type) {
        ...
        var data1 = result.data1;
        var data2 = result.data2;
        var xData = [], yData1 = [], yData2 = [], data2Max = 100, data2Min = Number(data2[0].value),
            data1Min = Number(data1[0].value), data1Max = Number(data1[0].value);
        for (var i = 0; i < data1.length; i++) {
        ...
        popShlAjzhpjzsfxChart = ec.init(document.getElementById('pop-shl-ajzhpjzsfx'));
        var option = {
            tooltip: {
                trigger: 'axis',
                axisPointer: {
    ...
    var drawPopHjwtfx = function (data) {
        var len = data.length;
        var xData = [], yData = [];
        var colorArr = ['#4068EC', '#4B89DE', '#53A4E5', '#63B2ED', '#73C3F4', '#7FE3FB'];
        var transparentData = {
            value: 0,
            name: "",
            itemStyle: {
    ...
    var drawPopAjwtdwfb = function (data) {
        var xData = [], yData = [], markData = [], startValue = 0, endValue = 100, isShow = false;
        if (data.length > 6) {
            isShow = true;
    ...
    var renderAjbljtwtfxDom = function (data) {
        ...
        $body.empty();
        for (var i = 0; i < data.length; i++) {
            $body.append('<div><div class="pop-view2-up">' + data[i].title +
                '</div><div class="pop-view2-down">' + data[i].content + '</div></div>');
        }
    };
    ...
    var initPage = function () {
        ...
        for (var i = 0; i < $div.length; i++) {
            getZsmyb(i, $div.eq(i).find('select').val());
        }
    };
    ...
    var initPopPage = function (index) {
        ...
        var difference = (parseInt(zxfxpjzsData[index] * 100) - parseInt(zxfxpjzsData[2].substring(0, zxfxpjzsData[2].length - 1) * 100)) / 100;
        var str2 = difference > 0 ? '高于' : '低于';
        difference = '' + Math.abs(difference);
        var str = str1 + '类案件评价指数<strong style="color: red;">' + str2 +
            '</strong>案件综合评价指数<strong style="color: red;">' + difference + '%</strong>';
    ...
    $('#hfrc').change(function () {
        ...
        if (value == 0) {
            ...
            for (var i = 0; i < hfrcData.length; i++) {
                container.append($('<div/>').append($('<span/>').html((i + 1) + '、' + hfrcData[i].name))
    ...
    // 监听列表办案小助手列表点击事件
    $('#pop-ajbljtwtfx, .view-mid-col-row-body').on('click', '.tab-td-click', function () {
        ...
        // 写入标题
        $('.pop-view2-header>span').html($(this).prev().prev().prev().html() + '  执行规范说明');
    ...
    $('.view-right-bottom-row-header').on('change', 'select', function () {
        getZsmyb($(this).parent().index('.view-right-bottom-row-header'), $(this).val());
    });
