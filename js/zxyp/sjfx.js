...
requirejs(['common', 'ec'], function (sugon, ec) {
    ...
    let functions = {
        ...
        initTop() { // 初始化上部
            sugon.requestJson({
                ...
                $('.sjfx-top strong').eq(0).html(result.data.dqsqzl);
                $('.sjfx-top strong').eq(1).html(result.data.pjmyd + '%');
                $('.sjfx-top strong').eq(2).html(result.data.rdqy);
            });
        },
        initMidLeft() { // 初始化中左
            sugon.requestJson({
                ...
                let chart = ec.init(document.getElementById('sjfx-chart1')), xData = [], yData = [];
                result.data.map(val => {
                    xData.push(val.name);
                        ...
                        areaStyle: {
                            color: {
                                ...
                                colorStops: [{
                                    ...
                                    offset: 1, color: '#70C4FD' // 100% 处的颜色
                                }],
                            }
                        }
                    }]
        ...
        initMidRightTop() { // 初始化中右上侧内容
            sugon.requestJson({
                ...
                result.data.map(val => {
                    body.append('<option value="' + val.id + '">' + val.value + '</option>');
                });
            });
        },
        initMidRight() { // 初始化中右
            ...
            sugon.requestJson({
                ...
                let $body = $('.time-box > ul').empty().css('margin-left', 0);
                let data = result.data.cluster;
                let len = data.length % 3 == 0 ? parseInt(data.length / 3) : parseInt(data.length / 3 + 1);
                let ulWidth = len * 100 + '%';
                let liWidth = 'calc((100% - ' + 40 * len + 'px) / ' + len * 3 + ')';
                $body.width(ulWidth);
                data.map(val => {
                    $body.append('<li style="width: ' + liWidth + '"><div><p>' + val.date + '\n' +
                        removeRichText(val.content1) + '</p></div><div><p>' + removeRichText(val.content2) + '</p></div></li>');
                });
            });
        },
        initBottomLeft() { // 初始化下左
            let $body = $('.sjfx-left-bottom').empty();
            $body.append('<div><div>诉求时间</div><div>姓名</div><div>诉求内容</div><div>分局</div></div>');
            sugon.requestJson({
                type: 'post',
                ...
                result.data.map(val => {
                    $body.append('<div><div>' + val.sqsj + '</div><div>' + val.xm + '</div><div>' + val.sqnr + '</div><div>' + val.fj + '</div></div>');
                });
            });
        },
        initBottomRight() { // 初始化下右
            ...
            sugon.requestJson({
                ...
                result.data.map((val, index) => {
                    $body.append('<div><div><span>' + (index + 1) + '、' + val.title + '</span><div><span>' + val.date +
                        '</span><span>' + val.dept + '</span></div></div><div><div><div>诉求内容</div><div>' + val.sqnr +
                        '</div></div><div><div>答复脚本</div><div>' + val.dfjb + '</div></div></div></div>');
                });
            });
        }
    };
    ...
    function timeLineEvent(e) {
        let $target = $(e.target);
        let $ul = $('.time-box > ul'), $body = $('.time-box'), len = $('.time-box > ul > li').length;
        let parentWidth = $body.width();
        let width = parentWidth * (len % 3 == 0 ? parseInt(len / 3) : parseInt(len / 3) + 1),
        ...
        if ($target.hasClass('left-btn')) {
            if (marginLeft < 0) {
                ...
                $ul.stop().animate({'margin-left': newMarginLeft + 'px'}, 200, 'linear', () => {
