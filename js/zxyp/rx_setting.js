...
requirejs(["common"], sugon => {
  function initLeft1() {
    ...
    sugon
      .request(sugon.interFaces.zxyp.rx.setting.left1, { deptId })
      .then(data => {
  ...
  function initLeft2(pageNum) {
    ...
    pageNum = pageNum || 1;
    sugon
      .request(sugon.interFaces.zxyp.rx.setting.left2, {
        pageNum,
        deptId,
        date1,
        date2,
  ...
  function initRight1(pageNum) {
    ...
    pageNum = pageNum || 1;
    sugon
      .request(sugon.interFaces.zxyp.rx.setting.right1, {
        deptId,
        date1,
        date2,
        pageNum
          ...
          if (index < 5) {
            $left.append(
              "<row><span>" +
                ...
                "、" +
                val +
                '</span><img src="../../img/zxyp/rx/del.png"></row>'
            );
          } else {
            $right.append(
              "<row><span>" +
  ...
  function initRight2(pageNum) {
    ...
    pageNum = pageNum || 1;
    sugon
      .request(sugon.interFaces.zxyp.rx.setting.right2, {
        deptId,
        date1,
        date2,
        pageNum
        ...
        data.data.map((val, index) => {
          $body.append(
            "<row><cell>" +
              ...
              "</cell><cell>" +
              val.col3 +
              '</cell><cell><img src="../../img/zxyp/rx/' +
              (val.col4 ? "like" : "dislike") +
              '.png"></cell></row>'
          );
        });
  ...
  // 添加按钮事件
  $(".add-btn").click(e => {
    let input = $(".add-event-input").val(),
      reg = /\S/;
    let { deptId, date1, date2 } = window.dialogParams;
    if (reg.test(input)) {
      sugon
        .request(sugon.interFaces.zxyp.rx.setting.addRight1, {
          name: input,
          deptId,
          date1,
          date2
