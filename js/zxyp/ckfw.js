requirejs(["common", "ec", 'jqcloud'], function (sugon, ec) {
    // 全局查询尺度
    var searchRuler = {}, search = {};
    var param1, param2;
    var hfrcData = [], gddwfbData = [], mydData = [];
    // 获取当前时间并加减固定月份
    ...
    var initMydfx = function (param) {
        sugon.requestJson({
            ...
            for (var i = 0; i < result.data.length; i++) {
                indicatorData.push({text: result.data[i].name, max: result.data[i].value});
...
//color: ['rgba(44, 193, 213, 1)', 'rgba(220, 214, 147, 1)', 'rgba(238, 172, 49, 1)', 'rgba(223, 102, 66, 1)', 'rgba(0, 246, 151, 1)', 'rgba(81, 86, 186, 1)'],
    ...
    var renderRankList = function () {
        ...
        data.map(function (val, index) {
            ...
            var $img, color = 'p';
            switch (index) {
                case 0: $img = '<img src="../../img/zxyp/jcj/one.png">';color = 'g';break;
                case 1: $img = '<img src="../../img/zxyp/jcj/two.png">';color = 'b';break;
                case 2: $img = '<img src="../../img/zxyp/jcj/three.png">';break;
                default: $img = index + 1;break;
            }
            var rankLab = $('<div/>').addClass('rankLab').append($('<div/>').append($img)).append($('<div/>').html(val.name));
            var rankImg = $('<div class="rankImg lh"></div>').addClass(color).html(val.value);
    ...
    var initTxt2 = function () {
        sugon.requestJson({
            ...
            for (var i = 0; i < result.data.length; i++) {
                $(".mid_title").eq(i).html(result.data[i].name);
                $(".val4").eq(i).html(result.data[i].value1);
                $(".val5").eq(i).html(result.data[i].value2);
                $(".val6").eq(i).html(result.data[i].value3);
                $(".val7").eq(i).html(result.data[i].value4);
            }
        });
    };
    ...
    var initWtyc = function (param) {
        sugon.requestJson({
            ...
            $(".wtList").empty();
            for (var i = 0; i < result.data.length; i++) {
                var str = result.data[i].name.length > 15 ? result.data[i].name.substr(0, 15) + "..." : result.data[i].name;
                var tempStr = (i + 1) + ". " + str;
                var tempImg = result.data[i].type == "1" ? "../../img/zxyp/ckfw/red.png" : "../../img/zxyp/ckfw/blue.png";
                $(".wtList").append(
                    "<div id=" + result.data[i].id + ">" +
                    "<span class='lh l tt'>" + tempStr + "</span>" +
                    "<span class='lh y advise'>建议</span>" +
                    "<span class='lh y img'><img src=" + tempImg + " /></span>" +
                    "</div>");
            }
        });
    };
