requirejs(["common", "ec"], function (sugon, ec) {
    ...
    var initMyd = function () {
        ...
        sugon.requestJson({
            ...
            var xData = [], yData = [], iData = [], min = result.data[0].value1, max = result.data[0].value1,
                min2 = result.data[0].value2, max2 = result.data[0].value2;
            for (var i = 0; i < result.data.length; i++) {
    ...
    var renderBsxn = function (data) {
        ...
        var yMin = Number(data[0].value2), yMax = Number(data[0].value2), xMin = Number(data[0].value1),
                ...
                formatter: function (obj) {
                    if (obj.componentType == "series") {
                        return '<div style="border-bottom: 1px solid rgba(255,255,255,.3); font-size: 18px;padding-bottom: 7px;margin-bottom: 7px">' +
                            obj.name + '</div><span>满意度</span>: ' + obj.data.value[0] + '%' +
                            '<br/><span>平均办理时长</span> : ' + obj.data.value[1];
                    }
                }
            },
            grid: {
    ...
    var renderYxal = function (type) {
        ...
        if (type == '0') {
            ...
            if (len % 2 == 0) {
                containerWidth = 'calc(' + len / 2 * 100 + '% + ' + (10 * len - 20) + 'px)';
                childWidth = 'calc(100% / ' + len + ' - 15px)';
            } else {
                containerWidth = 'calc(' + len / 2 * 100 + '% + ' + (10 * len - 10) + 'px)';
                childWidth = 'calc((100% + 15px) / ' + len + ' - 15px)';
            }
            $body.append('<div class="head-icon"><i class="glyphicon glyphicon-chevron-left"></i></div>');
            var $container = $('<div/>').addClass('mid-container');
            var $child = $('<div/>').css('width', containerWidth);
            yxalData.map(function (val, index) {
                $child.append('<div style="' + (index == 0 ? 'margin-left: 0;' : '') + 'width: ' + childWidth +
                    ';"><div class="col-row1" rowId="' + val.id + '" style="background: url(' + val.url + ') no-repeat;background-size: 100% 100%;">' +
                    '</div><div class="col-row23">' + val.content1 + '</div><div class="col-row23">单位:' + val.dept +
                    '</div><div class="col-row4">诉求内容：' + val.content2 + '</div></div>');
            });
            $container.append($child);
            $body.append($container);
            $body.append('<div class="head-icon"><i class="glyphicon glyphicon-chevron-right"></i></div>');
        } else {
            yxalData.map(function (val, index) {
                $body.append('<div class="row2"><div>' + (index + 1) + '、' + val.content1 + '</div><div>' +
                    val.date + ' ' + val.dept + '</div></div>');
            });
        }
    };
    ...
    var renderRdztDom = function (data) {
        ...
        $body.empty();
        for (var i = 0; i < data.length; i++) {
            $body.append('<div class="list-row" row-id="' + data[i].id + '">' +
                '<div>' +
                data[i].name +
                '</div>' +
                '<div>' +
                '<span>群众诉求：<strong class="strong-blue">' + data[i].value1 + '件</strong></span>' +
                '<span>满意度：<strong class="strong-green">' + data[i].value2 + '%</strong></span>' +
                '</div>' +
                '</div>');
        }
    };
        ...
        sugon.requestJson({
            ...
            for (var i = 0; i < result.data.length; i++) {
                ...
                $(".zfList").append(
                    ...
                    "<div class = '" + tempClass + "'>" + (i + 1) + "</div>" +
                    "<div>" + result.data[i].name + "</div>" +
                    "<div class='b'>" + result.data[i].tel + "</div>" +
                    "<div><img class='tempImg' src='../img/zxyp/jcj/file.png' /></div>" +
                    "</div>");
            }
            $(".tempImg").unbind().bind("click", function () {
                let keyword = $(this).parent().parent().attr('id');
    ...
    function preNextEvent(e) {
        var $doc = $('#yxal');
        var $target = $(e.target), $body = $('.mid-container'), $container = $('.mid-container > div'),
            containerWidth = $container.width(), width = $body.width() + 20, left,
            marginLeft = parseFloat($container.css('margin-left'));
        if ($target.hasClass('glyphicon-chevron-right') && Math.abs(marginLeft) + width <= containerWidth) {
            left = marginLeft - width;
            $container.stop().animate({'margin-left': left}, 200, 'linear', function () {
                $doc.one('click', '.head-icon > i', preNextEvent);
            });
        } else if ($target.hasClass('glyphicon-chevron-left') && marginLeft < -0.001) {
            left = marginLeft + width;
            $container.stop().animate({'margin-left': left}, 200, 'linear', function () {
                $doc.one('click', '.head-icon > i', preNextEvent);
            });
