...
requirejs(["common", "ec"], function(sugon, ec) {
  ...
  var id = params[1],
    searchRuler = JSON.parse(params[2]);
  searchRuler.id = id;
  // 诉求性质分析接口定义
  ...
  var drawPopMyd = function(data) {
    ...
    for (var i = 0; i < data.length; i++) {
      ...
      min =
        Math.min(data[i].value1, min) - 5 >= 0
          ? Math.min(data[i].value1, min) - 5
          : Math.min(data[i].value1, min);
      max = Math.max(data[i].value1, max);
      min2 =
        Math.min(data[i].value2, min2) - 5 >= 0
          ? Math.min(data[i].value2, min2) - 5
          : Math.min(data[i].value2, min2);
