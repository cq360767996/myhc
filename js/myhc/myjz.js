var myChart1, myChart2, myChart3, myChart4, Chart1, Chart2, Chart3, Chart4, Chart5, Chart6, Chart7, Chart8, Chart9, Chart10;
requirejs(["common"], function(sugon) {
    ...
    $("#mold").change(function() {
        ...
        if($(this).val() == "1") {
            $("#check").unbind().bind("click", function () {
                getList($('#placeCode').val(), $("#date1").val(), $("#date2").val(), $("#mold").val());
            });
            ...
            $("#check").unbind().bind("click", function () {
                getList2($('#placeCode').val(), $("#date1").val(), $("#date2").val(), $("#mold").val());
            });
    ...
    var getList2 = function(code, time1, time2, type) {
        sugon.requestJson({type: "POST", url: sugon.interFaces.myjz.bmList, data: {code: code, "time1": time1, "time2": time2, type: type}, async: false}, function (result) {
            var temp = "";
            if(result.type == "1") {
                temp = "jjdd"; //交通警察支队
            ...
            requirejs(['text!../views/myhc/' + temp + ".html", "myhc/" + temp], function (ele, fc){
                fc(result, ele);
            });
        });
    };
    ...
    var getList = function (code, time1, time2, type) {
        sugon.requestJson({
            ...
            for (var i = 0; i < result.data.length; i++) {
                ...
                var tempClass4 = '', tempClass5 = '', hbImg = '', tbImg = '';
                ...
                var imgName = '';
                switch (typeName) {
                    case "社会治安满意度" : imgName = "one"; break;
                    case "公安队伍满意度" : imgName = "two"; break;
                    case "社区民警熟悉率" : imgName = "three"; break;
                    case "110接处警满意度" : imgName = "five"; break;
                    case "12345满意度" : imgName = "four"; break;
                    case "交通事故处理评价指数" : imgName = "seven"; break;
                    case "案件办理评价指数" : imgName = "six"; break;
                    case "窗口服务满意度" : imgName = "eight"; break;
                    default : break;
                }
                var imgDiv = "<img class= 'imgClass' src='../../img/myhc/myjz/"+ imgName +".png'/>";

                $(".list").append(
                    "<div id=" + result.data[i].id + " type=" + result.data[i].type +" class=" + tempClass + ">" +
                    "<div class='list_content'>" +
                    "<div class='title'>" +
                    "<div class='name'><span class='" + tempClass1 + "'></span><span class='cName'>" + result.data[i].name + "</span></div>" +
                    // "<div class='" + tempClass3 + "'>" + result.data[i].date + "</div>" +
                    "</div>" +
                    "<div class='val'>" + imgDiv +
                    "<div class='" + tempClass2 + "'>" + result.data[i].value1 + "%</div>" +
                    "<div class='compare'>" +
                    "<div><span class='lab'>环比</span><span class='" + tempClass4 + "'>" + v3 + "</span><span>" + hbSpan + "</span></div>" +
                    "<div><span class='lab'>同比</span><span class='" + tempClass5 + "'>" + v4 + "</span><span>" + tbSpan + "</span></div>" +
                    "</div>" +
                    "</div>" +
                    "</div>" +
                    "</div>");
    ...
    $(document).on("click",".list>div", function() {
        ...
        var tempHtml = "";
        var F = $(this).find(".cName").html();
        $("#tempID").val(this.id + "_" + F);
        $("#tempTYPE").val($(this).attr('type'));

        if(F == "社会治安满意度" || F == "公安队伍满意度" || F == "社区民警熟悉率" || F == "12345满意度" || F == "110接处警满意度"){
