...
requirejs(["common", "ec"], function (sugon, ec) {
    ...
    sugon.requestJson({
        type: "POST",
        url: sugon.interFaces.myjz.Window1,
        data: {
            type: $("#tempTYPE").val(),
            id: $("#tempID").val(),
            code: $('#placeCode').val(),
        ...
        $(".b>div").eq(0).html(allData.data1[0].name + "：" + allData.data1[0].value1 + "%");
        $(".b>div").eq(1).html("同比：" + (allData.data1[0].value1 - allData.data1[1].value1).toFixed(2) + "%");
        $(".b>div").eq(2).html("环比：" + (allData.data1[0].value1 - allData.data1[2].value1).toFixed(2) + "%");

        $(".g>div").eq(0).html(allData.data1[1].name + "：" + allData.data1[1].value1 + "%");

        $(".p>div").eq(0).html(allData.data1[2].name + "：" + allData.data1[2].value1 + "%");
    });
    var yData = [];
    ...
    var initBar = function (index, txt) {
        ...
        if (allData.dwType == '1') {
            ...
            if ($("#tempTYPE").val() == '1' || $("#tempTYPE").val() == '2' || $("#tempTYPE").val() == '3') {
                suffix = "数值";
                titleTxt = '民意调查情况';
                $(".second-switch").hide();

            } else if ($("#tempTYPE").val() == '4' || $("#tempTYPE").val() == '6' || $("#tempTYPE").val() == '7') {
                suffix = "不满意工单数量";
                titleTxt = params[1].replace("满意度", "").replace("评价指数", "") + '不满意工单排序情况';
                $(".second-switch").hide();
            }
        }
