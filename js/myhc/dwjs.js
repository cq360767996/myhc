...
//----------------------------------------------------------------------------------------------------------------------
    ...
    var getNavigor = function(array) {
        ...
        for(var i=0;i<array.length;i++) {
            ...
            $(".navigor").append("<span class='nav' id="+ array[i].id +" >"+ array[i].name + tempStr +"</span>");
        }
        $(".navigor").append("<span>&nbsp;&nbsp;分析</span>");

        $(".nav").unbind().bind("click", function() {
    ...
    var initPie3D = function() {
        ...
        for(var i=0;i<oldData.length;i++) {
            ...
            for(var k=0;k<oldData[i].length;k++) {
                tempData.push({name: oldData[i][k].name, y: parseFloat(oldData[i][k].value)});
            ...
            $("div.title").eq(1).html(chooseStr+"数量分析");
            var ele = "pie_3D" + (i + 1);
    ...
    var initTopChart = function(result) {
        ...
        chart1.on('click', function(params) {
            initChart({name: params.data.name, id: params.data.id, value: params.data.value});
        });
    ...
    var initContent1 = function(data) {
        for(var i=0;i<data.length;i++) {
            $(".lab").eq(i).html(data[i].name);
            $(".val").eq(i).html(data[i].value2);
            $(".progress_val").eq(i).css("width", data[i].value1 + "%");
        }
    };
    ...
    var initContent2 = function(data) {
        ...
        var indicatorData = [], seriesData1 = [], seriesData2 = [], xData = [], yData = [], sum1 = 0, sum2 = 0;
        for(var i = 0; i < data.length; i++) {
        ...
        for(var i=0;i<data.length;i++) {
            ...
            indicatorData.push({text: data[i].name + "\n本期" + value1Percent + "%\n上期" + value2Percent + "%", max: 100});
    ...
    var initContent3 = function(data) {
        ...
        for(var i=0;i<data.length;i++) {
            ...
            $("#chart3").append(
                "<div>" +
                "<div class='pic'><img src="+ path +"></div>" +
                "<div class='name'>"+ data[i].name +"</div>" +
                "<div class='value'>"+ data[i].value +"条</div>" +
                "</div>");
        }
    };

    var getBar = function(data) {
        var xData = [], yData = [], startValue = 0, endValue = 100, markData = [];
        if(data.length > 4) {
    ...
    var initOtherCharts = function() {
        sugon.requestJson({
            ...
            data: {type: "dwjs", nav: JSON.stringify(navigorData),timestamp:timestamp,position:1, code: searchRule.code, date1: searchRule.date1, date2: searchRule.date2},
            async: true
        }, function (result) {
        ...
        sugon.requestJson({
            type: "POST",
            url: sugon.interFaces.dwjx.OtherCharts,
            data: {type: "dwjs", nav: JSON.stringify(navigorData),timestamp:timestamp,position:2, code: searchRule.code, date1: searchRule.date1, date2: searchRule.date2},
            async: true
        }, function (result) {
            if(result.timestamp == timestamp)
        ...
        sugon.requestJson({
            type: "POST",
            url: sugon.interFaces.dwjx.OtherCharts,
            data: {type: "dwjs", nav: JSON.stringify(navigorData),timestamp:timestamp,position:3, code: searchRule.code, date1: searchRule.date1, date2: searchRule.date2},
            async: true
        }, function (result) {
            if(result.timestamp == timestamp)
        ...
        sugon.requestJson({
            type: "POST",
            url: sugon.interFaces.dwjx.OtherCharts,
            data: {type: "dwjs", nav: JSON.stringify(navigorData),timestamp:timestamp,position:4, code: searchRule.code, date1: searchRule.date1, date2: searchRule.date2},
            async: true
        }, function (result) {
            if(result.timestamp == timestamp)
    ...
    var initChart = function(obj) {
        ...
        sugon.requestJson({
            ...
            data: {type: "dwjs", nav: JSON.stringify(navigorData),timestamp:timestamp, code: searchRule.code, date1: searchRule.date1, date2: searchRule.date2},
            async: true
        }, function (result) {
