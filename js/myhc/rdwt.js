...
requirejs(["common", "ec"], function (sugon, ec) {
    ...
    var getLiDom = function (index, data) {
        var result;
        if(index == 1) {
            result = $('<div/>').addClass('left-panel-li').addClass('left-panel-li-hover').attr('id', data.id).attr('mold', data.mold)
                .append($('<div/>').addClass('left-panel-li-header').html(index))
                .append($('<div/>').addClass('left-panel-li-body').html(data.content));
        } else {
            result = $('<div/>').addClass('left-panel-li').attr('id', data.id).attr('mold', data.mold)
                .append($('<div/>').addClass('left-panel-li-header').html(index))
                .append($('<div/>').addClass('left-panel-li-body').html(data.content));
        }
        return result;
    ...
    var getPagination = function (currentPage, totalPage) {
        ...
        var $ul = $(".pagination");
        $ul.empty();
        var first = $('<li/>').addClass('first-page').append($('<a/>').attr('href', 'javascript:void(0);').html('&laquo;'));
        var last = $('<li/>').addClass('last-page').append($('<a/>').attr('href', 'javascript:void(0);').html('&raquo;'));
        $ul.append(first);
        if (totalPage < 6) {
            for (var i = 0; i < totalPage; i++) {
                if (currentPage == i) {
                    ...
                    $ul.append($('<li/>').append($('<a/>').attr('href', 'javascript:void(0);').html((i + 1))));
                }
            }
        } else {
            switch (currentPage) {
                case 1 :
                    $ul.append($('<li/>').addClass('active').append($('<a/>').attr('href', 'javascript:void(0);').html(currentPage)))
                        .append($('<li/>').append($('<a/>').attr('href', 'javascript:void(0);').html(currentPage + 1)))
                        .append($('<li/>').append($('<a/>').attr('href', 'javascript:void(0);').html(currentPage + 2)));
                    break;
                case 2 :
                    $ul.append($('<li/>').append($('<a/>').attr('href', 'javascript:void(0);').html(currentPage - 1)))
                        .append($('<li/>').addClass('active').append($('<a/>').attr('href', 'javascript:void(0);').html(currentPage)))
                        .append($('<li/>').append($('<a/>').attr('href', 'javascript:void(0);').html(currentPage + 1)))
                        .append($('<li/>').append($('<a/>').attr('href', 'javascript:void(0);').html(currentPage + 2)));
                    break;
                case totalPage - 1 :
                    $ul.append($('<li/>').append($('<a/>').attr('href', 'javascript:void(0);').html(currentPage - 2)))
                        .append($('<li/>').append($('<a/>').attr('href', 'javascript:void(0);').html(currentPage - 1)))
                        .append($('<li/>').addClass('active').append($('<a/>').attr('href', 'javascript:void(0);').html(currentPage)))
                        .append($('<li/>').append($('<a/>').attr('href', 'javascript:void(0);').html(currentPage + 1)));
                    break;
        ...
        $ul.append(last);
        // 绑定点击分页栏的点击事件
        $(".pagination>li").unbind().bind('click', function () {
            var targetStr = $(this).find('a').html();
    ...
    var initSqxz = function (data) {
        ...
        var xData = [], yData = [], yPercentData = [], calcData = [];
        // 起始位置
        var sum = 0;
        for(var i = 0; i < data.length; i++) {
            xData.push(data[i].name);
        ...
        var seriesArr = [];
        var radiusArr = [[70, 75], [60, 70], [45, 60], [50, 45], [60, 50], [75, 60]];
        ...
        seriesArr.push({type: 'bar', data: [0], coordinateSystem: 'polar', name: '06a', stack: 'a'});
        var option = {
            color: ['#1D84C6', '#3A9BBE', '#ED7D31', '#FFC000', '#B08745', '#A770B3', '#6463AF'],
            tooltip: {
                show: true,
                formatter: "{b} <br/> {c} ({d}%)"
            },
    ...
    var initSjly = function (data) {
        $('#sjly-chart').empty();
        for(var i = 0; i < data.length; i++) {
            $('#sjly-chart').append($('<div/>').addClass('data-row')
                .append($('<div/>').addClass('data-row-header').html(data[i].name))
                .append($('<div/>').addClass('data-row-body').html((i == 0 ? (data[i].value + 69) : data[i].value) + ' 条')));
        }
    };
    ...
    var initDwfx = function (data) {
        var xData = [], yData = [], startValue = 0, endValue = 100;
        var isShow = false;
        if(data.length > 4) {
    ...
    var initRcfx = function (data) {
        ...
        for(var i = 0; i < data.length; i++) {
            if(i % 2 == 0) {
                $('#rcfx-chart').append($('<div/>').addClass('chart-row').css('background-color', '#CCDCEB')
                    .append($('<span/>').addClass('chart-row-header').html(data[i].name))
                    .append($('<span/>').addClass('chart-row-body').html(data[i].value + ' 次'))
                    .append($('<div/>').addClass('clear-div')));
            } else {
                $('#rcfx-chart').append($('<div/>').addClass('chart-row')
                    .append($('<span/>').addClass('chart-row-header').html(data[i].name))
                    .append($('<span/>').addClass('chart-row-body').html(data[i].value + ' 次'))
                    .append($('<div/>').addClass('clear-div')));
            }
    ...
    var loadLeftData = function (pageNum) {
        ...
        sugon.requestJson(ajaxOption, function (result) {
            if(!sugon.isPublished || searchRuler.timestamp == result.timestamp) {
                ...
                // 默认加载第一组数据
                loadRightData(resultData[0].id, resultData[0].mold, condition.deptId, condition.date);
            }
        });
    };
    ...
    $('.left-panel-body-header-person').click(function () {
        requirejs(['text!../views/myhc/ryqk.html'], function (ele){
            ...
            panel.html(ele).show().css('width', width).css('height', height);
        });
    });
    ...
    $('.download-report').click(function () {
        ...
        searchRuler.timestamp = condition.timestamp;
        var $form = $('<form />').attr('id', 'download-form').css('display', 'none')
            .attr('action', '/myrd/exportReport').attr('method', 'post');
        for(var key in condition) {
            $form.append($('<input type="hidden"/>').attr('name', key).attr('value', condition[key]));
        }
        $('body').append($form);
        $form.submit();
    ...
    $('.left-panel-body-header-setting').click(function () {
        location.hash = vipspa.stringify('rdwt/setting', {deptName : searchRuler.deptName, deptId : searchRuler.deptId, date : searchRuler.date});
    });
