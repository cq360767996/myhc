    ...
    (sugon, L, echarts, animate) => {
        ...
        function initFj(isMyzs) {
            ...
            requestMap(`DWBM = ${deptCode}`, true).then(serviceResult => {
                ...
                zoomArr[1] = {center, zoom: 12, upDeptCode: deptCode};
        ...
        function renderMyzsPcsPage(resultData) {
            ...
            sugon
                ...
                .then(async result => {
                    ...
                    pcsLayerGroup = L.layerGroup();
                    pcsMarkerGroup = L.layerGroup();
                    let data = [], heatMapData = [], sjPcsData = [];
                    let params = Object.assign(condition, {fjCode: sugon.identityInfo.deptCode});
                    await sugon.request(sugon.interFaces.myhc.myzs.getYlldPcsData, params).then(result => {
        ...
        // 初始化窗口面板
        function initCk() {
            sugon.request(sugon.interFaces.myhc.myzs.DeptId, {deptCode: sugon.identityInfo.deptCode}).then(result => {
                $("#placeCode").val(result.ID);
        ...
        function initCkBounds(index) {
            if (isLoadBounds) {
                requestMap(`DWBM like '${sugon.identityInfo.deptCode.substring(0, 6)}%' `).then(serviceResult => {
                    let resultData = serviceResult.result.features.features;
                    map.setView(zoomArr[1].center, zoomArr[1].zoom);
                    map.removeLayer(pcsLayerGroup);
                    pcsLayerGroup = L.layerGroup();
