requirejs(
    ["common", "L", "ec", "myzs_animate", "iclient", "heat", "markerCluster", "plot"],
    (sugon, L, echarts, animate) => {
        ...
        let initPopSetting3 = function (isRest) {
            ...
            sugon.request(sugon.interFaces.common.getDeptTree, {deptCode, role}).then(result => {
                $("#left-tree").treeview({
                    data: result.data,
                    levels: 1,
        ...
        // 单位层级由0 ==》 1
        async function level021(deptCode, upDeptCode, center, isMyzs) {
            let zoomArrObj = {};
            zoomArrObj.zoom = 12;
        ...
        // 单位层级由1 ==》 2
        async function level122(deptCode, upDeptCode, center, isMyzs) {
            let zoomArrObj = {};
            zoomArrObj.zoom = 14;
            zoomArrObj.upDeptCode = upDeptCode;
            if (center) {
        ...
        function initTab(type, pageNum, searchType) {
            ...
            $tab.empty().append(`<div class="data-tab-header">
                                    <div class="col1">序号</div>
                                    <div class="col2">时间</div>
                                    <div class="col3">诉求人</div>
                                    <div class="col4">${header1}</div>
                                    <div class="col5">${header2}</div
                                 </div>`);
        ...
        async function initCk() {
            await sugon.request(sugon.interFaces.myhc.myzs.DeptId, {deptCode: sugon.identityInfo.deptCode}).then(result => {
                $("#placeCode").val(result.ID);
                $("#place").val(result.NAME);
            });
            $(".search-down-panel > button")
                ...
                L.supermap
                    ...
                    .getFeaturesBySQL(sqlParam, function (serviceResult) {
                        ...
                        resultData.map(function (val, index) {
                            ...
                            resultLayer.on({
                                async click() {
                                    let code = val.properties.DWBM;
                                    let subCode = code.substring(0, 6);
                                    await sugon.request(sugon.interFaces.myhc.myzs.DeptId, {deptCode: code}).then(result => {
                                        $("#placeCode").val(result.ID);
                                        $("#place").val(result.NAME);
                                    });
        ...
        function reRenderChart(index) {
            index = index || 1;
            let data = [];
            let filterData = sqfxData.filter(val => val.dy == 0) || [];
            ...
            sugon.renderChart({id: "search-pop-chart", data, option});
        }
        ...
        function openCkPopup(code, layer, type) {
            sugon.requestJson(
                ...
                function (result) {
                    ...
                    let text =
                        !titleData ||
                        `{d|${titleData.name}}{a|满意度为}{c|${titleData.value}%} {a|全市排名}{b|${titleData.rank}}`;
                    let option = {
                        title: {
                            show: !!titleData,
                            text: text,
            ...
            sugon.requestJson(
                ...
                function (result) {
                    ...
                    data.map(function (value) {
                        str +=
                            `<div>
                                <div class="pop-col1">${value.gdbh}</div>
                                <div class="pop-col2">${value.xm}</div>
                                <div class="pop-col3" title="${value.dh}">${value.dh}</div>
                                <div class="pop-col4">${value.gdlb}</div>
                                <div title="${value.hfnr}" class="pop-col5">${value.hfnr}</div>
                             </div>`;
                    });
                    let ele = '<div class="map-mark-right-pop">' + str + "</div>";
                    sugon.renderDialog({
