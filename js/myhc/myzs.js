...
sugon.isPublished = false;
sugon.server = "../../";
sugon.interFaces = {
    myjz: {
        //查询栏日期默认值
        Date: {localUrl: "../../static/json/Date.json", remoteUrl: sugon.server + "/myjz/getRecentDate"}
    },
    myzs: {
        //下拉树
        Tree: {localUrl: "../../static/json/Tree.json", remoteUrl: sugon.server + "/myzs/getDwTree"},
        // 根据单位code获取单位id
        DeptId: {remoteUrl: sugon.server + "/myzs/getDeptId"},
        // 加载左侧数据
        getLeftData: {remoteUrl: sugon.server + '/myzs/getLeftData'},
        // 加载右侧中间部分数据
        getRightMidData: {remoteUrl: sugon.server + '/myzs/getRightMidData'},
        // 加载右侧数据
        getRightData: {remoteUrl: sugon.server + '/myrd/getRdwtList'},
        // 获取指标组成
        getZbzc: {localUrl: '../../static/json/myzs/pop-zbzs.json', remoteUrl: sugon.server + '/myzs/getZbzc'},
        // 指标展示悬停弹框
        getZbzcDetail: {
            localUrl: '../../static/json/myzs/pop-zbzs-detail.json',
            remoteUrl: sugon.server + '/myzs/getZbzcDetail'
        },
        // 执法公信力走势分析
        getZfgxl: {remoteUrl: sugon.server + '/myzs/getZfgxl'},
        // 单位情况
        getDwqk: {remoteUrl: sugon.server + '/myzs/getDwqk'},
        // 根据单位id获取单位层级
        getDwLevel: {remoteUrl: sugon.server + '/myzs/getDwLevel'},
        // 社会治安满意度调查情况
        getMyd: {localUrl: '../../static/json/myd.json', remoteUrl: sugon.server + 'myzs/getMyd'},
        // 获取数据录入初始化数据
        getSjlr: {localUrl: '../../static/json/myzs/pop1.json', remoteUrl: sugon.server + 'myzs/getSjlr'},
        // 提交数据录入
        submitSjlr: {localUrl: '../../static/json/myzs/success.json', remoteUrl: sugon.server + 'myzs/submitSjlr'},
        // 获取系数配置初始化数据
        getXspz: {localUrl: '../../static/json/myzs/pop2.json', remoteUrl: sugon.server + 'myzs/getXspz'},
        // 重置系数配置
        resetXspz: {localUrl: '../../static/json/myzs/pop2.json', remoteUrl: sugon.server + 'myzs/resetXspz'},
        // 提交系数配置
        submitXspz: {localUrl: '../../static/json/myzs/success.json', remoteUrl: sugon.server + 'myzs/submitXspz'},
        // 获取展示配置初始化数据
        getZspz: {localUrl: '../../static/json/myzs/pop3.json', remoteUrl: sugon.server + 'myzs/getZspz'},
        // 重置展示配置
        resetZspz: {localUrl: '../../static/json/myzs/pop3.json', remoteUrl: sugon.server + 'myzs/resetZspz'},
        // 计算展示配置数据
        calcZspz: {localUrl: '../../static/json/myzs/pop3.json', remoteUrl: sugon.server + 'myzs/calcZspz'},
        // 提交展示配置
        submitZspz: {localUrl: '../../static/json/myzs/success.json', remoteUrl: sugon.server + 'myzs/submitZspz'},
        getPoint: {localUrl: '../../static/json/myzs/success.json', remoteUrl: sugon.server + 'myzs/getTestJwd'},
        getMapData: {localUrl: '../../static/json/myzs/success.json', remoteUrl: sugon.server + 'myzs/getMapData'},
        getPcsZb: {localUrl: '../../static/json/myzs/success.json', remoteUrl: sugon.server + 'myzs/getPcsZb'},
        getDetailMapData: {
            localUrl: '../../static/json/myzs/success.json',
            remoteUrl: sugon.server + 'myzs/getDetailMapData'
        },
        getMapCount: {localUrl: '../../static/json/myzs/success.json', remoteUrl: sugon.server + 'myzs/getMapCount'},
        getMapDataByBounds: {
            localUrl: '../../static/json/myzs/success.json',
            remoteUrl: sugon.server + 'myzs/getMapDataByBounds'
        },
        getCkfw: {
            localUrl: '../../static/json/myzs/success.json',
            remoteUrl: sugon.server + 'myzs/getCkfw'
        },
        getCkMapData: {
            localUrl: '../../static/json/myzs/success.json',
            remoteUrl: sugon.server + 'myzs/getCkMapData'
        },
        getCkfwq: {
            remoteUrl: sugon.server + 'myzs/getCkfwq'
        },
        getCkDetail: {
            remoteUrl: sugon.server + 'myzs/getCkDetail'
        },
        getPointByCode: {
            remoteUrl: sugon.server + 'myzs/getPointByCode'
        },
        getCkRdwtDetail: {
            remoteUrl: sugon.server + 'myzs/getCkRdwtDetail'
        },
        getCkfwqDetail: {
            remoteUrl: sugon.server + 'myzs/getCkfwqDetail'
        },
        getCkfwqZb: {
            localUrl: "../../static/json/Date.json",
            remoteUrl: sugon.server + 'myzs/getCkfwqZb'
        },
        getCkfwqRank: {
            localUrl: "../../static/json/Date.json",
            remoteUrl: sugon.server + 'myzs/getCkfwqRank'
        }
    }
};
sugon.requestJson = function (urlOrInterfaceObj, callBack) {
    var url;
    if (typeof urlOrInterfaceObj.url == "string") {
        url = urlOrInterfaceObj.url;
    } else {
        url = sugon.isPublished ? urlOrInterfaceObj.url.remoteUrl : urlOrInterfaceObj.url.localUrl;
    }
...
sugon.showDialog = function (option) {
...
// 提示框组件
sugon.showMessage = function (msg, type) {
    var divClass, iClass;
    switch (type) {
        case 'warning':
    ...
    $('body').append('<div class="pop-message ' + divClass + '">' +
...
// 分页组件
sugon.renderNav = function ($div, pageNum, totalPage, callback) {
    ...
    $('.com-nav > div').unbind().bind('click', function (e) {
        ...
        sugon.renderNav($div, toPage, totalPage, callback);
        callback(toPage);
    });
};
...
var initList = function () {
    var date = '';
    sugon.requestJson({type: "POST", url: sugon.interFaces.myjz.Date, async: false}, function (result) {
        date = result.data;
    });
    return date;
};
...
var getRightMidData = function () {
    ...
    sugon.requestJson(ajaxObj, function (result) {
        var data1 = result.data1, data2 = result.data2;
...
var getRightDownData = function () {
    ...
    sugon.requestJson(ajaxObj, function (result) {
        ...
        for (var i = 0; i < data.length; i++) {
            $body.append('<div title="' + data[i].content + '">' + (i + 1) + '、' + data[i].content + '</div>');
        }
    });
};
...
var renderDwqk = function (data) {
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        if (data[i].selected) {
            markData = [
                {name: data[i].name, value: data[i].value, xAxis: -1, yAxis: data[i].value},
                {name: data[i].name, value: data[i].value, xAxis: data.length, yAxis: data[i].value}
            ]
        }
    }
    min = Number(min - (max - min) / 2).toFixed(2);
...
var initPopPage2 = function () {
    sugon.requestJson({
        ...
        if (searchRuler.type == 1) {
            $('.pop-panel2-header').html('社会治安满意度调查情况<i class="glyphicon glyphicon-remove"></i>');
        } else {
            $('.pop-panel2-header').html('公安队伍满意度调查情况<i class="glyphicon glyphicon-remove"></i>');
        }
    ...
    sugon.requestJson({
        ...
        result.data.map(function (val1, i) {
            val1.map(function (val2, j) {
                var index = i * 12 + j;
                $('.tab-cell > input').eq(index).val(val2);
            })
        });
    });
};
...
var initPopSetting2 = function (isRest) {
    sugon.requestJson({
        type: 'post',
        url: isRest ? sugon.interFaces.myzs.resetXspz : sugon.interFaces.myzs.getXspz,
        data: {date: $('#pop-date2').val()}
    }, function (result) {
        result.data.map(function (val, index) {
            $('.right-tab1 > div > input').eq(index).val(val);
        });
    });
};
...
var initPopSetting3 = function (isRest) {
    sugon.requestJson({
        type: 'post',
        url: isRest ? sugon.interFaces.myzs.resetZspz : sugon.interFaces.myzs.getZspz,
        data: {
        ...
        result.data1.map(function (val, index) {
            $('.num-div').eq(index).html(val + '%');
        });
        result.data2.map(function (val, index) {
            $('.forth-col1 > div').eq(index).html(val + '%');
        });
        result.data3.map(function (val, index) {
            if (index == 3 || index == 10 || index == 11 || index == 13 || index == 14) {
                $('.right-tab2 > div > input').eq(index).val(val);
            } else {
                $('.right-tab2 > div > input').eq(index).val(val);
            }
        });
...
var submitPopSetting1 = function () {
    ...
    for (var i = 0; i < 13; i++) {
        ...
        for (var j = 0; j < 12; j++) {
            var index = i * 12 + j;
            var str = $('.tab-cell > input').eq(index).val();
            innerArr.push(str);
        }
        arr.push(innerArr);
    }
...
var submitPopSetting2 = function () {
    ...
    for (var i = 0; i < $('.right-tab1 > div > input').length; i++) {
        arr.push($('.right-tab1 > div > input').eq(i).val());
    }
    var condition = {
        time: $('#pop-date2').val(),
        data: JSON.stringify(arr)
...
$('.right-panel-up-1-up > div').click(function () {
    ...
    if (index == 0) {
        $('.pop-panel').show();
        $('.pop-panel > div > span').html($('#place').val() + '执法公信力指数分析');
        initPopPage();
    } else {
        $('.pop-panel2').show();
...
$('.setting-btn').click(function () {
    ...
    if (display == 'block') {
        ...
        $selectTab.css('left', position.left - 50 + 'px');
        $selectTab.css('top', position.top + 40 + 'px');
...
$('.select-option').click(function () {
    ...
    switch (index) {
        case 1:
            ...
            break;
        case 2:
            $('#pop-date2').val($('#date2').val().slice(0, 4));
            initPopSetting2();
            break;
        case 3:
...
$('#pop-dept-name').click(function () {
    var position = $(this).offset();
    var $tree = $('#left-tree');
    $tree.css('visibility', 'visible').css('left', position.left + 'px')
...
$('.body-row-body').mouseover(function (e) {
    ...
    sugon.requestJson({
        ...
        result.data.map(function (val) {
            $ul.append('<li><div>' + val.name + '</div><div>' + val.value + '</div></li>');
        });
    });
});
...
$('.body-row-body').mousemove(function (e) {
    $('.inner-pop-panel').css('left', e.clientX - 100);
    $('.inner-pop-panel').css('top', e.clientY + 20);
});
...
var zrqLayerGroup = L.layerGroup(), zrqMarkerGroup = L.layerGroup(); // 责任区layer相关数据
var singleZrqLayerGroup = L.layerGroup(), singleZrqMarkerGroup = L.layerGroup(); // 单个责任区layer相关数据
var heatLayerGroup = [], currentLevel = ''; // 聚合点数据
...
function createMapL() {
    ...
    mapLayer = L.supermap.wmtsLayer("http://10.33.66.183:2333/iserver/services/map-wmts-GADT/wmts-china",
        {
            layer: "GADT",
            style: "default",
...
function getAllFjData() {
    ...
    L.supermap.featureService(url).getFeaturesBySQL(sqlParam, function (serviceResult) {
        ...
        resultData.map(function (val) {
            ...
            var center = L.latLngBounds(changeLonAndLat(val.geometry.coordinates)).getCenter();
            resultLayer.on({
        ...
        sugon.requestJson({
            ...
            var mapParams = '{"code": "' + result.ID + '", "codeName": "' + result.NAME +
                '", "mold": "1", "mapCode": "' + result.CODE + '"}';
    ...
    fjData.map(function (val) {
        ...
        var divIcon = L.divIcon({
            className: 'fj-icon',
            html: '<div class="bubblefj" code="' + val.code + '" style="width: ' + len + 'px;">' + val.name + '</div>',
            iconAnchor: [len / 2, 0]
        });
...
function mouseoverEvent(e) {
    ...
    map.eachLayer(function (layer) {
        ...
        if (layer._leaflet_id == e.target._leaflet_id) {
            ...
            $div.css('color', '#ffe200').stop().animate({"top": "5px"}, 200, function () {
...
function level122(deptCode, upDeptCode, center) {
    ...
    // 隐藏左右两侧页面
    $('.right-panel-up').show().css('height', '50%');
    ...
    sugon.requestJson({
        ...
        var nameArr = ['110接处警满意度', '窗口服务满意度', '案件综合评价', '事故综合评价',
            '12345满意度', '社区民警熟悉率', '社会治安满意度', '公安队伍满意度'];
        $body.empty();
        result.data.map(function (val, index) {
            if (val) {
                var html = '<div><div class="right-amount2">' + val + '</div><div class="right-name2">'
...
function getPcsDataByFjCode(fjCode) {
    ...
    var sqlParam = new SuperMap.GetFeaturesBySQLParameters({
        queryParameter: {
            name: 'gajg_pcszrq_3201_pg@ORCL_gt8',
            attributeFilter: "DWBM like '" + subCode + "%' and DWMC like '%派出所'",
            fields: ["DWBM", "DWMC"],
            orderBy: "DWBM"
        },
        datasetNames: ['ORCL_gt8:gajg_pcszrq_3201_pg'],
    ...
    L.supermap.featureService(url).getFeaturesBySQL(sqlParam, function (serviceResult) {
        ...
        resultData.map(function (val) {
            ...
            var divIcon = L.divIcon({
                className: 'pcs-icon',
                html: '<div class="bubblefj" code="' + val.properties.DWBM + '" style="width: ' + len + 'px;">' + val.properties.DWMC + '</div>',
                iconAnchor: [len / 2, 0]
            });
                ...
                mouseup: function (e) {
                    ...
                    if (which == 3) {
                        ...
                        sugon.requestJson({
                            ...
                            var mapParams = '{"code": "' + result.UP_ID + '", "codeName": "' + result.UP_NAME +
                                '", "mold": "1", "mapCode": "' + result.UP_CODE + '"}';
                            sessionStorage.setItem('mapParams', mapParams);
                            getAllFjData();
                        });
                        // 重设中心点和zoom值
...
function getZrqDataByPcsCode(pcsCode) {
    ...
    var sqlParam = new SuperMap.GetFeaturesBySQLParameters({
        queryParameter: {
            name: 'gajg_pcszrq_3201_pg@ORCL_gt8',
            attributeFilter: "DWBM like '%" + subCode + "%' and DWMC like '%责任区'",
            fields: ["DWBM", "DWMC"],
            orderBy: "DWBM"
        },
        datasetNames: ['ORCL_gt8:gajg_pcszrq_3201_pg'],
    ...
    L.supermap.featureService(url).getFeaturesBySQL(sqlParam, function (serviceResult) {
        ...
        resultData.map(function (val) {
            ...
            var divIcon = L.divIcon({
                className: 'zrq-icon',
                html: '<div class="bubblefj" code="' + val.properties.DWBM + '" style="width: ' + len
                    + 'px;">' + dwmc + '</div>',
                iconAnchor: [len / 2, 0]
...
function initTab(type, pageNum, searchType) {
    ...
    $tab.empty();
    $tab.append('<div class="data-tab-header"><div class="col1">序号</div>' +
        '<div class="col2">时间</div><div class="col3">诉求人</div><div class="col4">'
        + header1 + '</div><div class="col5">' + header2 + '</div</div>');
    ...
    queryGroup = L.layerGroup();
    data.map(function (value, index) {
        var $div = $('<div/>').addClass('data-panel-tab-row').attr('ywid', value.ywid);
        $div.append($('<div/>').addClass('col1').html(index + 1));
        $div.append($('<div/>').addClass('col2').html(value['sj']));
        $div.append($('<div/>').addClass('col3').html(value['sqr']));
        $div.append($('<div/>').addClass('col4').html(value['sjly']));
        $div.append($('<div/>').addClass('col5').html(value['ywlx']));
        $tab.append($div);
        var icon;
        ...
        switch (value.sjly) {
            case '110接处警':
                ...
                icon = L.icon({iconUrl: '../../img/myhc/myzs/location_aj.png', popupAnchor: [10, 5]});
                break;
            case '案件':
                ...
                icon = L.icon({iconUrl: '../../img/myhc/myzs/location_jcj.png', popupAnchor: [10, 5]});
                break;
            case '12345热线':
                str += '<p>姓名：' + (value.sqr || '') + '</p>';
                str += '<p>电话：' + (value.dh || '') + '</p>';
                ...
                icon = L.icon({iconUrl: '../../img/myhc/myzs/location_12345.png', popupAnchor: [10, 5]});
                break;
        }
        var resultLayer = L.marker([value.lat, value.lng], {icon: icon});
        resultLayer.bindPopup('<div ywid="' + value.ywid + '" class="pop-map-mark-header">'
            + value.sjly + '</div><div class="pop-map-mark-body">' + str + '</div>');
        resultLayer.on('popupopen', function (e) {
            ...
            if (ywid) {
                var tabRow = $('.data-panel-tab-row'), className = 'data-panel-tab-row-hover';
    ...
    var totalCount = pageStr.substring(pageStr.indexOf('(') + 1, pageStr.lastIndexOf(')'));
    var pages = totalCount % 100 == 0 ? parseInt(totalCount / 100) : parseInt(totalCount / 100) + 1;
...
function initCkMark(deptCode, isNotZoomIn) {
    ...
    sugon.requestJson({
        ...
        data.map(function (value) {
            ...
            var width = value.name.length * 16 + 40, top = '';
            ...
            var html = '<div code="' + value.code + '" style="width: ' + width + 'px;">' + top +
                '<div class="pop-mid pop-ck-mid"><div class="pop-mid">' + value.value + '%</div><div class="pop-mid">'
                + value.name + '</div></div><div class="pop-ck-down ' + className + '"></div></div>';
            var divIcon = L.divIcon({
                className: 'ck-popup',
                html: html,
...
function initCkRight(deptCode, isPcs) {
    ...
    sugon.requestJson({
        ...
        if (data && data.length > 0) {
            ...
            $banner.append('<div class="right-up-banner-left">' + data[0][1] + '%</div>' +
                '<div class="right-up-banner-right"><div><span>同比</span><div class="'
                + colorIcon1.color + '"><i class="glyphicon ' + colorIcon1.icon + '"></i><strong>'
                + Math.abs(data[0][2]) + '%</strong></div></div><div><span>环比</span><div class="'
                + colorIcon2.color + '"><i class="glyphicon ' + colorIcon2.icon + '"></i><strong>' + Math.abs(data[0][3])
                + '%</strong></div></div><div><span>办理量</span><div class="banner-color3"><strong>'
                + data[0][4] + '</strong></div></div></div>');
            data.map(function (value, index) {
                if (index > 0) {
                    var img = '';
                    ...
                    var str;
                    if (!isPcs) {
                        str = '<div class="down-left" id="' + img + '-chart"></div><div class="down-right">' +
                            '<span>办理量</span><strong class="banner-color3">' + value[4] + '</strong></div>';
                    } else {
                        str = '<div class="down-left" style="width: 100%;"><strong style="color: #3286ff;">' +
                            value[1] + '%</strong><span>全局：<span style="color: #3286ff;">' + value[5]
                            + '%</span>&nbsp;&nbsp;&nbsp;' + (value[6] ? ('排名：<span style="color: #3286ff;">' +
                                value[6] + '</span>/12') : '') + '</span></div>';
                    }
                    var html = '<div><div><div class="up-left"><img src="../../img/myhc/myzs/' + img + '.png"><span>'
                        + value[0] + '</span></div><div class="up-right"><span>同比</span><i class="' + colorIcon1.color +
                        ' glyphicon ' + colorIcon1.icon + '"></i><strong class="' + colorIcon1.color + '">' + Math.abs(value[2]) +
                        '%</strong><span>环比</span><i class="' + colorIcon2.color + ' glyphicon ' + colorIcon2.icon +
                        '"></i><strong class="' + colorIcon2.color + '">' + Math.abs(value[3]) +
                        '%</strong></div></div><div>' + str + '</div>';
                    $body.append(html);
...
function loadHeatMap() {
    ...
    sugon.requestJson({
        ...
        var heatLayer = [];
        heatLayer.push(L.heatLayer(result.data, {
            radius: 10,
            maxOpacity: .7,
            featureWeight: 'value',
            gradient: {
...
function renderSearchChart() {
    ...
    sugon.requestJson({
        ...
        data1.map(function (val, index) {
            $('.left-mid-amount').eq(index).html(val);
        });
        reRenderChart();
    });
}
...
function reRenderChart(index) {
    ...
    var xData = [], yData = [], min = 0, max = 0, isSet,
...
function symbolLibInitializeCompleted() {
    ...
    SuperMap.Plot.PlottingUtil.getDataFromServer(serverUrl, libID, code, null, {}, null,
        function (res) {
            ...
            if (!bounds || JSON.stringify(bounds) != JSON.stringify(newBounds)) { // 判断边界非空或者相等
                bounds = newBounds;
            }
            // 加载图形数据
            sugon.requestJson({
                ...
                result.data.map(function (value) {
                    ...
                    queryPlottingLayer.createSymbol(libID, code, latLngs, value.ywid, {
                        color: 'transparent'
...
function openCkPopup(code, layer, type) {
    sugon.requestJson({
        ...
        var data1 = result.data1, data2 = result.data2, titleData;
        var rightData = '', xData = [], yData = [], min = 0, max = 0, diff;
        ...
        data2.map(function (value, index) {
            rightData += '<div><div title="' + value.content + '">' + (index + 1) + '、' + value.content +
                '</div><span code="' + value.code + '">（' + value.count + '）</span></div>';
        });
        ...
        var html = '<div class="pop-map-mark-header">满意度分析</div>' +
            ...
            'id="marker-chart-' + random + '"></div><div class="map-mark-right"><div deptCode="' +
            code + '" class="map-mark-right-title"><span style="float: left;">本期热点问题：</span>' +
            '<span style="float: right;">问题量</span></div><div class="map-mark-right-body">'
            + rightData + '</div></div></div>';
        var popupOption = {};
        if (type == 1) {
            ...
            series: [
                {
                    ...
                    label: {
                        ...
                        lineHeight: 16,
                        formatter: function (param) {
                            var inc = '', rank = param.data.rank ? '{a|全局第}{c|' + param.data.rank + '}\n' : '';
                            if (param.data.inc) {
                                if (param.data.inc > 0) {
        ...
        var chart = echarts.init(document.getElementById('marker-chart-' + random));
        chart.setOption(option);
    });
}
...
function openCkPulsePopup(code, layer) {
    ...
    sugon.requestJson({
        ...
        var data = result.data;
        var xData = [], data1 = [], data2 = [], rightData = '', min = 0, max = 0, diff;
        if (data && data.length > 0) {
            min = data[0].qj;
            max = data[0].bdw;
        }
        ...
        var html = '<div class="pop-map-mark-header">满意度分析</div>' +
            ...
            'id="marker-chart-' + random + '"></div><div style="float: left;width: 50%">' +
            '<div class="map-mark-right-body2">' + rightData + '</div></div></div>';
        layer.bindPopup(html).openPopup().unbindPopup();
        var chart = echarts.init(document.getElementById('marker-chart-' + random));
        var option = {
            tooltip: {
                show: true
...
function initPulse(index) {
    sugon.requestJson({
        ...
        data.map(function (value) {
            ...
            marker.on({
                click: function () {
                    openCkPulsePopup(value.code, marker);
                },
                mouseover: function () {
                    map.removeLayer(popMarkerGroup);
                    popMarkerGroup = L.layerGroup();
                    var width = value.name.length * 16 + 30;
                    var html = '<div class="pop-marker-bg" style="width: ' + width + 'px;">' + value.name + '</div>';
...
function searchFunc() {
    ...
    if (reg.test(inputVal)) {
        ...
        $('.keyword-search').html('包含“<span style="color: #1d84c6;">' + searchInput + '</span>”的群众诉求共计：');
        var type = 0;
        var searchYw = $('.search-yw');
        var searchGj = $('.search-gj');
        ...
        sugon.requestJson({
            ...
            var totalCount = result.data[0].substring(result.data[0].indexOf('(') + 1, result.data[0].lastIndexOf(')'));
            if (totalCount < 50) {
                $('.search-pop-panel').hide();
                removeAllLayers();
                initDataPanel(type);
...
document.addEventListener('click', function (e) {
    ...
    if ($target.hasClass('pop-mid')) {
        ...
        var codeExist = false, index;
        for (var key in ckGroup._layers) {
            if (ckGroup._layers[key].options.icon.options.html.indexOf(code) != -1) {
                codeExist = true;
                index = key;
                break;
            }
...
$('.analysis-btn').click(function () {
    ...
    $container.show().empty().append($('<div class="pop-map-mark-header">区域热点问题<i class="glyphicon glyphicon-remove"></i></div>'));
    var size = result.length < 4 ? result.length : 3;
    for (var i = 0; i < size; i++) {
        $body.append('<p ywid="' + result[i].id + '">' + (i + 1) + '、该区域' + result[i].name + '问题较为突出。    ' + result[i].value + '件</p>');
    }
...
$('.toolbar-panel2 > div').on('click', function () {
    ...
    $('.toolbar-panel2-pop').attr('type', index).show().css('width', width).css('left', left).css('top', offset.top + 47);
    initCkBounds(index);
    if (!syncPulse) {
        initPulse(index);
    }
...
$('#mainMap').on('click', '.map-mark-right-body > div > span', function () {
    ...
    sugon.requestJson({
        ...
        str += '<div class="map-mark-right-pop-header"><div class="pop-col1">工单编号</div><div class="pop-col2">姓名</div><div class="pop-col3">电话</div><div class="pop-col4">工单类别</div><div class="pop-col5">回访内容</div></div>';
        data.map(function (value) {
            str += '<div><div class="pop-col1">' + value.gdbh + '</div><div class="pop-col2">' +
                value.xm + '</div><div class="pop-col3">' + value.dh + '</div><div class="pop-col4">' +
